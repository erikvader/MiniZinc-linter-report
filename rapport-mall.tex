...
For example $x_1 > x_2$ or $x_3 = 3$ etc. When all constraints are satisfied is a
...
targets, then it will stop searching that path and backtrack to an earlier point and try a
different path.
...
instance, then a warning is issued, since if that parameter is changed might these rewrites not
...
case must \mi{b} and \mi{c} be included in the search annotation, it is optional for
\mi{a} to be included.
...
constrained together with \mi{forall} is this approximation not too incorrect.
...
defined, then the corresponding argument is also functionally defined. In the example above (Listing~\ref{lst:count}) is
\mi{c} functionally defined in the definition of \mi{count}, that means that \mi{z} in the
function call \mi{count(zs, 2, z)} also is functionally defined. The definitions are
